...
import com.riandy.flexiblealertscheduling.AlarmContract.Alarm;
...
public class AlarmDBHelper extends SQLiteOpenHelper {
	...
	private static final String SQL_CREATE_ALARM = "CREATE TABLE " + Alarm.TABLE_NAME + " (" +
			...
			Alarm.COLUMN_NAME_ALARM_START_DATE + " TEXT, " +
			Alarm.COLUMN_NAME_ALARM_END_DATE + " TEXT " +
	    " )";
	...
	private AlarmModel populateModel(Cursor c) {
		...
		model.startDate = new Date(c.getString(c.getColumnIndex(Alarm.COLUMN_NAME_ALARM_START_DATE)));
		model.endDate = new Date(c.getString(c.getColumnIndex(Alarm.COLUMN_NAME_ALARM_END_DATE)));
//        Log.d("SQL AlarmDBHelper startDate getting",model.startDate.toString()+ " "+model.endDate.toString());
	...
	private ContentValues populateContent(AlarmModel model) {
        ...
        values.put(Alarm.COLUMN_NAME_ALARM_START_DATE, model.startDate.toString());
        values.put(Alarm.COLUMN_NAME_ALARM_END_DATE, model.endDate.toString());        
        String repeatingDays = "";
        for (int i = 0; i < 7; ++i) {
        	repeatingDays += model.getRepeatingDay(i) + ",";
        }
        values.put(Alarm.COLUMN_NAME_ALARM_REPEAT_DAYS, repeatingDays);
//        Log.d("SQL AlarmDBHelper startDate storing",model.startDate.toString() + " " + model.endDate.toString());
        return values;
	}
